  ...
  if (!versions.length) {
    ...
    const compatible = all.filter(
      (v) =>
        ...
        (v.gameVersions.includes(mcVersion) || v.gameVersions.some((gv) => gv.startsWith(mcVersion.slice(0, 4))))
    )
    return compatible[0] ?? null
  }
  return versions.find((v) => v.releaseType === 'release') ?? versions[0] ?? null
}
